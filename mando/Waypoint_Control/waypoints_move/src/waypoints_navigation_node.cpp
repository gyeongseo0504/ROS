...
double current_imu_angle_target = 0.0;
...
struct Line_equation Waypoints_line_equation[WayPoints_NO];
...
void gps_utm_poseCallback(const geometry_msgs::Pose2D& msg)
{
    ...
    // use_utm_absolute_mode에 따라 좌표 변환 조건부 적용
    if(use_utm_absolute_mode) {
        // UTM 절대 모드: 90도 회전 없이 그대로 사용
        my_pose_map.x = my_pose.x;
        my_pose_map.y = my_pose.y;
    } else {
        // 상대 모드: 기존 90도 회전 유지
...
void init_waypoints(const std::string& file_path)
{
	...
	else
	{
		...
		for(int i=0; i<no_waypoints-1; i++)
		{
			...
			Waypoints_line_equation[i].a_utm     = a;
			Waypoints_line_equation[i].b_utm     = b;			
			Waypoints_line_equation[i].theta     = waypoint_line_angle ;
			Waypoints_line_equation[i].theta_utm = waypoint_line_angle_utm ;
			
		}
		
		Waypoints_line_equation[no_waypoints-1] = Waypoints_line_equation[no_waypoints-2];
...
void base_link_tf_utm(void)
{
    ...
    tf_base_map_x = -my_pose.x;
    tf_base_map_y = -my_pose.y;
    tf_base_map_x += my_target_pose_goal.x;
    tf_base_map_y += my_target_pose_goal.y;
    
    if(use_utm_absolute_mode) {
        // UTM 절대 모드: 일반적인 변환
        waypoint_pos_base_link_x = tf_base_map_x * cos(my_pose.theta) + tf_base_map_y * sin(my_pose.theta);
        waypoint_pos_base_link_y = -tf_base_map_x * sin(my_pose.theta) + tf_base_map_y * cos(my_pose.theta);
    } else {
        // 기존 방식: 90도 회전 변환
        waypoint_pos_base_link_y = (tf_base_map_x * cos(my_pose.theta) + tf_base_map_y * sin(my_pose.theta)) * -1;
...
visualization_msgs::Marker Draw_Marker_point(float x, float y)
{
    ...
    if(use_gps_init_datum == true) {
        ...
        if(use_utm_absolute_mode) {
            x_p = x - my_waypoints_list[0].x;
            y_p = y - my_waypoints_list[0].y;
        } else {
            x_p = y - my_waypoints_list[0].y;
            y_p = -(x - my_waypoints_list[0].x);
        }
    }
...
void imu_heading_angle_Callback(const std_msgs::Float32& msg)
{
	...
	{
		imu_heading_angle = msg.data + M_PI;
...
double virtual_xte_calculation(int wp_id, double *px_utm , double *py_utm)
{
	...
	double a = Waypoints_line_equation[wp_id-1].a_utm;
	double b = Waypoints_line_equation[wp_id-1].b_utm;
	
	double a1 = -1/Waypoints_line_equation[wp_id-1].a_utm; //tan(gps_heading_angle );
	double b1 = 0;	
	//double a2 = 0;
...
visualization_msgs::Marker Make_utm_circle_Marker(double x, double y,float scale, float r, float g, float b, float a = 1.0 )
{
    visualization_msgs::Marker marker;

    marker.header.frame_id = "utm";  // 적절한 프레임 ID로 변경하세요
    ...
    marker.pose.position.z = 0;  // 2D이므로 z는 0으로 설정
    ...
    // 크기 설정 (미터 단위)
    marker.scale.x = scale;  // 지름
    marker.scale.y = scale;  // 지름
    marker.scale.z = 0.01; // 높이 (얇게 설정)
    ...
    marker.lifetime = ros::Duration();  // 0은 영구적
...
int main(int argc, char **argv)
{
	...
	char buf[2];
	ros::init(argc, argv, "one_fifth_waypoints_manager_utm");
	...
	double way_point_ahead_distance_old = way_point_ahead_distance;
	...
	ros::param::get("~cross_track_offset",cross_track_offset);
	...
	ros::Subscriber lidar_obs_flag_sub                  = n.subscribe("/obstacle/lidar_avoid_detect_status",1, &lidar_obs_detect_Callback);
	...
	ros::Publisher car_yaw_control_pub                  = n.advertise<std_msgs::Float32>("/Car_Control_Cmd/Target_Angle", 1);
	...
	ros::Publisher marker_current_point_pub             = n.advertise<visualization_msgs::Marker>("/marker/current_point",1,true);
	...
	ros::Publisher wp_line_equation_pub                 = n.advertise<std_msgs::Float32MultiArray>("/wp/wp_line_equation", 1);
	...
	bool   car_stop_flag         = false;
	
	WayPoints my_virtual_sensor_position;
	Line_equation my_line_map;
		...
		if(run_flag == 1)
		{
			...
			if(waypoint_id!= -1)
			{
			  ...
			  //printf("%d %d \n", wp_go_id, no_waypoints);
				double current_utm_x, current_utm_y;
				if(use_gps_init_datum == true) 
				{
					//current_y =  -(my_pose.x - utm_datum_pose.x);   //map 
					//current_x =   (my_pose.y - utm_datum_pose.y);
					current_utm_x =   (my_pose.x - utm_datum_pose.x);
					current_utm_y =   (my_pose.y - utm_datum_pose.y);
					printf("    my_pose : %6.3lf %6.3lf | %6.3lf %6.3lf \n", my_pose.x , my_pose.y,  current_utm_x , current_utm_y);
				}
				else
				{
					//current_y = -(my_pose.x - gps_init_pose2d_data.x);  //map 
					//current_x =  (my_pose.y - gps_init_pose2d_data.y); 
					
					current_utm_x =  (my_pose.x - gps_init_pose2d_data.x);
					current_utm_y =  (my_pose.y - gps_init_pose2d_data.y);
					//printf("   gps_init_pose2d_data: %6.3lf %6.3lf\n",gps_init_pose2d_data.x, gps_init_pose2d_data.y);
					printf("    my_pose : %6.3lf %6.3lf | %6.3lf %6.3lf \n", my_pose.x , my_pose.y,  current_utm_x , current_utm_y);
				}
				
				visualization_msgs::Marker current_pos_marker;
				
				current_pos_marker = Make_utm_circle_Marker(current_utm_x, current_utm_y, 0.4, 1.0, 1.0, 0.0);
				marker_current_point_pub.publish(current_pos_marker);
			...
			if(wp_go_id >=1)
			{
				std_msgs::Float32MultiArray wp_line_equation_msg;
				wp_line_equation_msg.data.push_back(Waypoints_line_equation[wp_go_id-1].a_utm);  // 배열 요소 1
				wp_line_equation_msg.data.push_back(Waypoints_line_equation[wp_go_id-1].b_utm);  // 배열 요소 2
				wp_line_equation_pub.publish(wp_line_equation_msg);
							
				std_msgs::Float32 wp_line_angle;
				wp_line_angle.data = Waypoints_line_equation[wp_go_id-1].theta_utm; 
			...
			{
				std_msgs::Float32MultiArray wp_line_equation_msg;
				wp_line_equation_msg.data.push_back(Waypoints_line_equation[0].a_utm);  // 배열 요소 1
				wp_line_equation_msg.data.push_back(Waypoints_line_equation[0].b_utm);  //  배열 요소 2
				wp_line_equation_pub.publish(wp_line_equation_msg);				
				
				std_msgs::Float32 wp_line_angle;
				wp_line_angle.data = Waypoints_line_equation[0].theta_utm;
				waypoint_guide_angle_pub.publish(wp_line_angle);
			...
			// waypoint_gap_distance은 목적 waypoint에서 waypoint_gap_distance거리 앞에서 종료할 것 인지를 결정함
			// way_point_ahead_distance = 0이면 waypoint 에서 멈춤


			if( (count>=0) && ( my_pose_utm_waypoint.x  >= waypoint_gap_distance )  && ( wp_go_id <= wp_finish_id)) 
			{           
				...
				if(new_wp_flag == true)
				{
					...
					wp_go_id--;
					printf("-----------------------------------\n"); 
					printf("Arrived at Obstacle WayPoint[%3d] !\n",wp_go_id); 
					printf("-----------------------------------\n"); 
			...
			{	
				...
				{/*
					...
					{					
						...
						avoid_control_set_pub.publish(avoid_set);
						c_speed.data = 0.0;
						car_control_pub2.publish(c_speed);  // 정지 명령을 보낸다.
						car_control_pub2.publish(c_speed);
						temp_speed = c_speed.data;
						ros::Duration(3).sleep();  // 경사로 3초 정지인데 혹시 모르니 4초 정지
					...
					{					
						...
						car_control_pub2.publish(c_speed);
						temp_speed = c_speed.data;
						ros::Duration(3).sleep();  // 신호등 3초 정지

						wp_go_id = 87;
					}	
					
					
					if((wp_go_id >= 87) && (wp_go_id <= 163)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
						avoid_set.data = false;
					...
					{					
						...
						lane_control_set_pub.publish(lane_set);
						avoid_control_set_pub.publish(avoid_set);
						c_speed.data = 1.0; // 추후 속도 정할 것
						temp_speed = c_speed.data;
					}
					if((wp_go_id >= 183) && (wp_go_id <= 187)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
						avoid_set.data = false;
					...
					{					
						...
						lane_control_set_pub.publish(lane_set);
						avoid_control_set_pub.publish(avoid_set);
						c_speed.data = -0.9; // 추후 속도 정할 것
						temp_speed = c_speed.data;
					}
					if((wp_go_id >= 194) && (wp_go_id <= 196)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
						avoid_set.data = false;
					...
					{					
						...
						lane_control_set_pub.publish(lane_set);
						avoid_control_set_pub.publish(avoid_set);
						c_speed.data = 0.8; // 추후 속도 정할 것
						temp_speed = c_speed.data;
					}
					if((wp_go_id >= 201) && (wp_go_id <= 261)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
						avoid_set.data = false;
...
/*
					...
					{					
						...
						lane_control_set_pub.publish(lane_set);
						avoid_control_set_pub.publish(avoid_set);
						c_speed.data = 1.3; // 추후 속도 정할 것
						temp_speed = c_speed.data;
					}
					
					// 굴곡라인
					if((wp_go_id >= 164) && (wp_go_id <= 168)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
						avoid_set.data = false;
					...
					{					
						...
						lane_control_set_pub.publish(lane_set);
						avoid_control_set_pub.publish(avoid_set);
						c_speed.data = 1.8; // 추후 속도 정할 것
						temp_speed = c_speed.data;
					}
					
					// S자 라인 주행 및 신호등 전까지 주행을 위한 약간의 감속
					if((wp_go_id >= 105) && (wp_go_id <= 170)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
						avoid_set.data = false;
					...
					{					
						...
						car_control_pub2.publish(c_speed);
						temp_speed = c_speed.data;							
						ros::Duration(3).sleep();  //정지 후 3초 멈춤 후에 100번으로 출
						wp_go_id = 194;
					}
					
					// T자 주차 후진
					if((wp_go_id >= 194) && (wp_go_id <= 205)) 
					{
						steering_control_mode = 0;
						first_run = false;   // 
						forward_run = false;
						way_point_ahead_distance = way_point_ahead_distance_old;   // 다시 정지 거리 복귀
					...
					{					
						...
						car_control_pub2.publish(c_speed);
						temp_speed = c_speed.data;							
						ros::Duration(10).sleep();  //정지 후 3초 멈춤 후에 100번으로 출
						wp_go_id = 207;
					}
					
					// T자 주차 완료 후 신호등 전까지 주행
					if((wp_go_id >= 207) && (wp_go_id <= 276)) 
					{			
						//first_run = true;   // 
						forward_run = true;
					...
					// 평행 주차 후 마지막 종료 라인으로 주행
					if((wp_go_id >= 399) && (wp_go_id <= 435)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
						avoid_set.data = false;
					...
					if(wp_go_id == 600)
					{
						current_imu_angle     = imu_heading_angle;						
						current_imu_angle_target = current_imu_angle + M_PI_4;
						if(current_imu_angle_target >= M_PI) current_imu_angle_target = current_imu_angle_target - M_PI*2 ;
					...
					if(wp_go_id == 601)
					{
						...
						steering_control_mode = 2; // 강제로 steering을 움직임	
						steering_control_mode_msg.data = steering_control_mode;					
						steering_control_mode_pub.publish(steering_control_mode_msg);	
						printf(" 0-5 steering_control_mode %d \n", steering_control_mode);
						
						
					    car_steering_angle = -25;
					    car_target_angle.data = car_steering_angle;
					    car_yaw_control_pub.publish(car_target_angle);
					    
					    c_speed.data = -0.7;
					    car_control_pub2.publish(c_speed);  
					    printf("angle1: %6.3lf , angl2 : %6.3lf\n", RAD2DEG(current_imu_angle), RAD2DEG(current_imu_angle_target));
					    if( fabs(imu_heading_angle - current_imu_angle_target) < DEG2RAD(3.0))
					    {
							c_speed.data = 0.0;
							temp_speed = c_speed.data;							
							car_control_pub2.publish(c_speed);  // 정지 명령을 보낸다.
							...
							ros::Duration(1).sleep();  //정지 후 1초 멈춤 후에 302번으로 출발
							wp_go_id = 602;
						}					    
					...
					if(wp_go_id == 602)
					{
						...
						car_control_pub2.publish(c_speed);  // 정지 명령을 보낸다.
						car_control_pub2.publish(c_speed);
						ros::Duration(0.8).sleep();  //정지 후 1초 멈춤 후에 302번으로 출발
					...
					*/
				    ...
				    if(wp_go_id == 603)
					{
						...
						car_steering_angle = 23;
					    car_target_angle.data = 23;
					    car_yaw_control_pub.publish(car_target_angle);
					    car_yaw_control_pub.publish(car_target_angle);
						...
						if( fabs(imu_heading_angle - current_imu_angle)  < DEG2RAD(3.0))
					    {
							c_speed.data = 0.0;
							temp_speed = c_speed.data;							
							car_control_pub2.publish(c_speed);  // 정지 명령을 보낸다.
					...
					{
						steering_control_method = 2;
						steering_control_mode = 0; // target_yaw로 원위치 
						steering_control_mode_msg.data = steering_control_mode;					
						steering_control_mode_pub.publish(steering_control_mode_msg);	
						printf(" 0-5 steering_control_mode %d \n", steering_control_mode);
						wp_go_id = 399;  // 복귀
			...
			else // 라이다 장애물 검출이 되면...
			{
				...
				car_steering_angle = 0;
				car_target_angle.data = 0;
				car_yaw_control_pub.publish(car_target_angle);
				car_yaw_control_pub.publish(car_target_angle);
				c_speed.data = 0.0;   // obstacle detection speed (reduce speed)
			...
			*/
			...
			{
				case 0 :
    car_steering_angle = RAD2DEG(steering_angle_base);  // ← 이 줄 추가!
    ...
    break;
		...
		{ 
		  ...
		  // Points are blue
			...
			{
				start_p.x =  (my_pose.y - my_waypoints_list[0].y); 
				start_p.y = -(my_pose.x - my_waypoints_list[0].x);
