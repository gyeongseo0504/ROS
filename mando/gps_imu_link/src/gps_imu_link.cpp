...
{
	ros::init(argc, argv, "imu_gps_heading_angle_link");
	ros::NodeHandle n;
	...
	ros::Subscriber sub_enable_imu_correction     = n.subscribe(imu_correction_enable__topic, 1,&imu_correction_enable_Callback);
	...
	while (ros::ok())
	{
		...
		if( (shared_gps_fix1_topic != NULL) && (shared_gps_fix2_topic != NULL) )
		{
			...
			{
				if( (auto_imu_offset_flag == true) && ( enable_imu_correction == true) )
				{
					printf("Auto IMU Offset Mode! \n");
				
				}
				else
				{ 
					printf("Disable IMU Offset Mode! \n");
				}
				...
				if( enable_imu_correction == true)
				{
					//imu_offset          = gps_heading_angle_radian - corrected_imu_angle_radian;					
					imu_offset            = gps_heading_angle_radian - imu_heading_angle_radian;					
				}
				
				imu_offset_angle_degree.data = RAD2DEG(imu_offset);
				...
				if(auto_imu_offset_flag == true)
				{
					imu_yaw_offset_degree = RAD2DEG(imu_offset);
				}
				
				corrected_imu_angle_radian = imu_heading_angle_radian + DEG2RAD(imu_yaw_offset_degree);
				printf("corrected_imu_angle : %6.3lf\n", RAD2DEG(corrected_imu_angle_radian) ); 
			
			    yaw_error = RAD2DEG(gps_heading_angle_radian)-RAD2DEG(corrected_imu_angle_radian);
			    printf("yaw_error : %6.3lf \n", yaw_error);
			    
	           	if(auto_imu_offset_flag == true)
				{	
					printf("Auto IMU Offset Mode! \n");
					if(fabs(yaw_error) > 3.0)
					{
					  imu_heading_angle_msg.data =  imu_heading_angle_radian + imu_offset;
					  printf("Auto Correction done !!\n");			
				    }
				    else
				    {
   				...
   				{
					imu_heading_angle_msg.data = imu_heading_angle_radian  + DEG2RAD(imu_yaw_offset_degree);
				}
				...
				if( enable_imu_correction == true)
				{
					printf("final auto gps/ imu heading /error : [%6.3lf | %6.3lf | %6.3lf ]\n\n\n",RAD2DEG(gps_heading_angle_radian), RAD2DEG(imu_heading_angle_msg.data), yaw_error);
				}
				else
				{
					printf("final         / imu heading /error : [%6.3lf | %6.3lf ]\n\n\n", RAD2DEG(imu_heading_angle_msg.data), yaw_error);
				}		
			...
			else
			{
				...
				if( enable_imu_correction == false)
				{
					imu_heading_angle_msg.data =  imu_heading_angle_radian + imu_offset;
					imu_heading_angle_radian_pub.publish(imu_heading_angle_msg);
				}
