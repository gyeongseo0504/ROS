...
{
	ros::init(argc, argv, "imu_gps_heading_angle_offset_test");
	ros::NodeHandle n;
	...
	while (ros::ok())
	{
		...
		if( (shared_gps_fix1_topic != NULL) && (shared_gps_fix2_topic != NULL) )
		{
			...
			if( (gps_fix1 == 2) &&(gps_fix2 == 2) )
			{
				...
				//sum_offset += imu_offset;
				//cnt++;		
		        
				//imu_offset = sum_offset/cnt;
		        
				imu_offset_angle.data = imu_offset;
				imu_yaw_offset_degree = RAD2DEG(imu_offset);
		
				printf("gps heading : %6.3lf | imu heading : %6.3lf | imu offset : %6.3lf \n",RAD2DEG(gps_heading_angle_radian),RAD2DEG(imu_heading_angle_radian), RAD2DEG(imu_offset));
					
				//imu_offest_angle_radian_pub.publish(imu_offset_angle);
				...
				double yaw_error = RAD2DEG(gps_heading_angle_radian)-RAD2DEG(imu_heading_angle_msg.data);
				printf("final gps/ imu heading : [%6.3lf | %6.3lf | %6.3lf ]\n\n\n",RAD2DEG(gps_heading_angle_radian), RAD2DEG(imu_heading_angle_msg.data), yaw_error);
				
				imu_heading_angle_radian_pub.publish(imu_heading_angle_msg);
				
				if( yaw_error >=5.0 ) 
				{
					ROS_ERROR("Yaw Offset is too big");
					sleep(4);
				}
			}
