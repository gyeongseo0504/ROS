...
double imu_heading_angle_radian = 0.0;
...
void imu_heading_angle_radianCallback(const std_msgs::Float32& msg)
{
	imu_heading_angle_radian = msg.data;            
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_IMU_heading_angle  = n.subscribe(imu_heading_angle_radian_topic, 20, &imu_heading_angle_radianCallback);  // imu
    
  
  
  ros::Publisher gps_angle_pub        = n.advertise<std_msgs::Float32>("/gps/heading_angle",1); 
  ros::Publisher gps_angle_degree_pub = n.advertise<std_msgs::Float32>("/gps/heading_angle_degree",1); 
  ...
  ros::Publisher gps_odom_pub = n.advertise<nav_msgs::Odometry>("/odom/nav",1); 
  ...
  {	
	...
	if((fix1 == 2) && (fix2 == 2) )  // if 2 gps's are all fixed
	{
	   ...
	   if(use_two_gps == 1)
	   {
			...
			gps_angle_degree_pub.publish(gps_heading_angle_degree);
	   }    
   ...
   else if(fix1 == 2)   // if 1 gps's are all fixed
   {
		...
		if(use_imu_yaw_angle == 1) h_angle = imu_heading_angle_radian;	
        ...
        ROS_INFO("Front GPS E: %.7lf N: %.7lf",Pose1.x,Pose1.y);
        ROS_INFO("Heading Angle : %.7lf",RAD2DEG(h_angle));      
        printf("imu_angle : %6.3lf\n", RAD2DEG(imu_heading_angle_radian));
	    utm1_pub.publish(utm1);
   }
       ...
       odom_quat = tf::createQuaternionMsgFromYaw(imu_heading_angle_radian);//yaw trans quat
