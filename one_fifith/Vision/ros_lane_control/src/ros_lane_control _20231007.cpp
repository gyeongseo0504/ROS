...
int main(int argc, char** argv)
{
	...
	*/
	...
	std::string cmd_vel_vision_output_topic = "/cmd_vel/vision";
	std::string input_image_topic           = "/camera/color/image_raw";
	...
	ros::param::get("~cmd_vel_vision_output_topic",cmd_vel_vision_output_topic);
	if (USE_CAMERA == 0) img_height = 480;

    std::cout << "OpenCV version : " << CV_VERSION   << std::endl;
	std::cout << "Image Widht  : "   << img_width    << std::endl;
	std::cout << "Image Height : "   << img_height   << std::endl;
	std::cout << "ROI_CENTER_Y : "   << ROI_CENTER_Y << std::endl;
	std::cout << "ROI_WIDTH : "      << ROI_WIDTH    << std::endl;
	
 	std::cout << "canney_low  : "    << canney_low   << std::endl;
	std::cout << "canney_high : "    << canney_high  << std::endl;
	
	std::cout << "cmd_vel_vision_output_topic : " <<cmd_vel_vision_output_topic << std::endl;
		...
		else
		{
			...
			*/
			...
			if(right_line_cnt !=0)
			{
				...
				line(mat_image_roi_crop, Point(x1, y1), Point(x2, y2), Scalar(255, 255, 255), 2, LINE_AA);
			...
			if( (left_line_cnt!=0) && (right_line_cnt !=0))
			{
				...
				line(mat_image_roi_crop, Point(line_center,0), Point(line_center, ROI_WIDTH*2),  Scalar(  255,  0, 0), 1, LINE_AA);
			}
			...
			cross_track_error_msg.data = line_center - img_width/2;
			
			printf("Lane Cross Track Error : %6.3lf\n",cross_track_error_msg.data);
			cross_track_error_pub.publish(cross_track_error_msg);
			
			error_lane = cross_track_error_msg.data;
			error_lane_delta = error_lane - error_lane_old;
			
			steer_angle_new = -(error_lane *Kp + Kd*error_lane_delta)/M_PI;
			
			steer_angle_new = (steer_angle_new >  MAX_STEER_ANGLE) ?  MAX_STEER_ANGLE : steer_angle_new;
			steer_angle_new = (steer_angle_new < -MAX_STEER_ANGLE) ? -MAX_STEER_ANGLE : steer_angle_new;
			
			std_msgs::Int16  vision_steering_angle_msg;
						
			if(lane_control_run_flag == true) 
			{
				vision_steering_angle_msg.data = steer_angle_new; 
			}
			else
			{
				vision_steering_angle_msg.data = 0;
			}
			
			vision_car_control_pub.publish(vision_steering_angle_msg);
