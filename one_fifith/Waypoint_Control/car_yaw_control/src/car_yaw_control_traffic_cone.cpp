...
double Kp_traffic_cone    = 0.01;
double Kd_traffic_cone    = 0.04;
double Ki_traffic_cone    = 0.00;
...
void traffic_cone_target_yaw_Callback(const std_msgs::Float32& msg)
{
	...
	traffic_cone_target_yaw = RAD2DEG(msg.data) ;
...
double traffic_cone_control_yaw(void)
{
	...
	*/
	...
	steering_angle_control = Kp_traffic_cone * error + Kd_traffic_cone * error_d + Ki_traffic_cone * error_sum;
	...
	{
		printf("steering_control_mode : %d\n",steering_control_mode);
		ROS_INFO("IMU Yaw Angle : %6.3lf %6.3lf Target Yaw : %6.3lf Error : %6.3lf | steer angle %6.3lf",imu_heading_angle_d,imu_heading_angle_d_360,traffic_cone_target_yaw,error,steering_angle_control );
	}
...
double control_yaw(void)
{
	...
	*/
	...
	{
		steering_angle_control = steering_angle_control * -1.0;
	}
...
int main(int argc, char **argv)
{
	...
	std::string imu_yaw_angle_topic                = "/robor/imu/yaw_radian";
	...
	ros::param::get("~Kp_traffic_cone", Kp_traffic_cone);  
	ros::param::get("~Kd_traffic_cone", Kd_traffic_cone);  
	ros::param::get("~Ki_traffic_cone", Ki_traffic_cone);  
	...
	*/ 
	...
	while(ros::ok())
	{
		  ...
		  if(control_action_flag == 1)
		  {
			...
			if (steering_control_mode == 8)  //traffic cone control
			{
