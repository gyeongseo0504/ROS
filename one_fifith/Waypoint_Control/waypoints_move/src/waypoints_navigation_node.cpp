...
#define MAX_L_STEER -20
#define MAX_R_STEER 20
...
void publishWaypointMarkers()
{
    ...
    for (int i = 0; i < no_waypoints; i++) {
        ...
        m.pose.position.x = my_waypoints_list[i].x;
        m.pose.position.y = my_waypoints_list[i].y;
        m.pose.position.z = 0.0;
        m.pose.orientation.w = 1.0;
        m.scale.x = 0.3; m.scale.y = 0.3; m.scale.z = 0.3;

        if (i < wp_go_id) {         // 지난 웨이포인트
            m.color.r = 1.0; m.color.g = 1.0; m.color.b = 0.0; m.color.a = 1.0;
        } else if (i == wp_go_id) { // 현재 목표 웨이포인트
...
void WaySteerControlCallback(const std_msgs::Int16& angle)
{
	...
	if(waypoint_steering_angle >= MAX_R_STEER)  
	{
	  waypoint_steering_angle = MAX_R_STEER;
	}

	if(waypoint_steering_angle <= MAX_L_STEER)  
	{
	  waypoint_steering_angle = MAX_L_STEER;  
	}
}
...
int main(int argc, char **argv)
{
	...
	marker_pub = n.advertise<visualization_msgs::MarkerArray>("/marker/waypoint", 1);
