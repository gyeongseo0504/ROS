...
int main(int argc, char **argv)
{
	...
	std::string waypoint_file_name = "/home/kyeongseo/one_fifth_catkin_ws/waypoints/pre.txt";
	...
	{	
		...
		if(run_flag == 1)
		{
			...
			if((wp_go_id <= 26 && lidar_obs_flag == false) || wp_go_id > 26)
			{	
				if(fix1 == 2)
				{	
					// 출발 지점엥서부터 시작
					if((wp_go_id >= 0) && (wp_go_id <= 11)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
					...
					{					
						...
						temp_speed = c_speed.data;
						ros::Duration(4).sleep();  // 경사로 3초 정지인데 혹시 모르니 4초 정지
					...
					// 정지 후 경사로 오르막길 통과를 위한 가속
					if((wp_go_id >= 19) && (wp_go_id <= 20)) 
					{					
						steering_control_mode = 0;
						lane_set.data = false;
						avoid_set.data = false;
					...
					if(wp_go_id == 603)
					{
						...
						if( fabs(imu_heading_angle - current_imu_angle)  < DEG2RAD(3.0))
						{
							...
							car_control_pub2.publish(c_speed);  // 정지 명령을 보낸다.
							car_control_pub2.publish(c_speed);
							ros::Duration(4).sleep();  //정지 후 1초 멈춤 후에 302번으로 출발
							wp_go_id = 604;
						}
					}
